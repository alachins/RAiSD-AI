...
class SweepNet1DV2(torch.nn.Module):
    def __init__(self, H, W, channels):
        super(SweepNet1DV2, self).__init__()
        ...
        out_conv1 = self.compute_out_shape((H, W), conv1_kernel, conv1_stride)
        out_pool1 = self.compute_out_shape(out_conv1, pool1_kernel, pool1_stride)
        out_conv2 = self.compute_out_shape(out_pool1, conv2_kernel, conv2_stride)
        out_pool2 = self.compute_out_shape(out_conv2, pool2_kernel, pool2_stride)

        # init output layers
        self.fc1 = torch.nn.Linear(conv2_channels, 32)
        self.out_relu = torch.nn.ReLU()
        self.out_pool = torch.nn.AvgPool2d((1, out_pool2[1]))
    ...
    def forward(self, x):
        ...
        self.conv1.weight.data = self.conv1.weight.data[:, :, 0, :].unsqueeze(2).repeat((1, 1, self.conv1.weight.data.shape[2], 1))
        self.conv2.weight.data = self.conv2.weight.data[:, :, 0, :].unsqueeze(2).repeat((1, 1, self.conv2.weight.data.shape[2], 1))
        
        x = self.conv1(x)
        x = self.pool1(x)
        x = self.relu1(x)
        
        x = torch.reshape(x, (x.shape[0], x.shape[1], 1, x.shape[3]))
...
class SweepNet1DV1(torch.nn.Module):
    
    def __init__(self, H, W, outputs, channels):
        super(SweepNet1DV1, self).__init__()
...
class SweepNet1DV4(torch.nn.Module):
    
    def __init__(self, H, W, outputs, channels):
        super(SweepNet1DV4, self).__init__()
